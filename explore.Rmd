...
df_comp <- read_rds("data/df_comp.rds") %>% 
  fmutate(across(c("avg_hrs", "rep_comp", "rep_comp_rltd"), 
                 as.numeric)) %>% 
  fsubset(avg_hrs >= 5 & ## select those who have worked at least 5 hrs per week and
           rep_comp >=5000)  ## paid more than $5000/year by the reporting organization  
...
brd_mem_titles <- c("BOARD MEMBER", "TREASURER", "SECRETARY", "BOARD",
                   ...
                   "CHAIR", "SEC/TREAS", "VICE CHAIRMA", "SECRETARY AND TREASURER"
                   )
...
df_comp2 <- df_comp2 %>%
  ...
  title = str_remove_all(title, "CO "),  ## remove co from title of co-leaders
  
  # title = if_else(title  %in% brd_mem_titles, "BOARD MEMBER", title),
  ...
  fmutate(title = if_else(str_like(title, "DIRECTOR%"), ## change director x to x director. 
          ...
          title = if_else((title == "PRESIDENTEXECUTIVE DIRECTOR" |
                            title == "CEOPRESIDENT" |
                             ...
                             title == "PRESIDENTAND CEO"), "PRESIDENT AND CEO", title), 
          ...
          title = if_else(title == "COOCFO" |
                            title == "CFOCOO" |
                            title == "COO AND CFO", "CFO AND COO", title),
